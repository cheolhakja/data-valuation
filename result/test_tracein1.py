...
np.savetxt(out_path2, np.column_stack([idx, scores]), fmt=["%d", "%.8f"])
print(f"Saved: {out_path2}")
#===============================저장용===============================
score_a = scores[0:20000].sum()
score_b = scores[20000:40000].sum()
score_c = scores[40000:60000].sum()

total = score_a + score_b + score_c
ratio_a, ratio_b, ratio_c = score_a/total, score_b/total, score_c/total

with open("contributors_summary.txt", "w", encoding="utf-8") as f:
    f.write(f"A_sum={score_a:.6f}, ratio={ratio_a:.6f}\n")
    f.write(f"B_sum={score_b:.6f}, ratio={ratio_b:.6f}\n")
    f.write(f"C_sum={score_c:.6f}, ratio={ratio_c:.6f}\n")

print("Saved: contributors_summary.txt")
